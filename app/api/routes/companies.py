...
def create_company(company: CompanyCreate, db: Session = Depends(get_db),_:User=Depends(require_admin)):
    
    existing_company = (db.query(Company).filter(Company.name ==company.name,Company.is_deleted == False).first())
...
def get_companies(page: int = Query(1, ge=1),limit:int =Query(10, ge=1, le=100),db: Session =Depends(get_db),search: Optional[str] = None,sort_by:str =Query("id"),order: str =Query("asc")):
    offset = (page -1) *limit
    query = (db.query(Company).filter(~Company.is_deleted))
...
def update_company(company_id: int, company:CompanyUpdate,db:Session =Depends(get_db),_: User =Depends(require_admin)):
    company_to_update = (
    db.query(Company)
    .filter(
        Company.id == company_id,
...
def delete_companies(company_id: int, db: Session=Depends(get_db), _: User =Depends(require_admin)):
    company_to_delete = db.query(Company).filter(Company.id == company_id, Company.is_deleted == False).first()
    

    if company_to_delete is None:
        raise HTTPException(status_code=404, detail="Company not found")

    company_to_delete.is_deleted = True
    company_to_delete.deleted_at = utc_now()
